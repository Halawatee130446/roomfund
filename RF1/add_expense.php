...
include 'header.php';
?>
<link rel="stylesheet" href="assets/add_expense.css">
<div class="form-card">
    <h2>➕ Add New Expense</h2>

    <?php if (!empty($error)) echo "<div class='flash-error'>".htmlspecialchars($error)."</div>"; ?>

    <form method="post" action="add_expense.php" class="styled-form">
        <input type="hidden" name="csrf" value="<?php echo csrf_token(); ?>">
        
        <label for="expense_date">Date:</label>
        <input type="date" name="expense_date" id="expense_date" required value="<?php echo htmlspecialchars($_POST['expense_date'] ?? date('Y-m-d')); ?>">

        <label for="description">Description:</label>
        <input name="description" id="description" required value="<?php echo htmlspecialchars($_POST['description'] ?? ''); ?>">
        ...
        <input type="number" name="amount" id="amount" step="0.01" required value="<?php echo htmlspecialchars($_POST['amount'] ?? ''); ?>">
