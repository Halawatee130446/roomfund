<?php
// header.php (เวอร์ชันแก้ไข - Top Bar แสดงผลในหน้า Login/Register แล้ว)
...
// ตรวจว่าหน้าปัจจุบันคือ login/register หรือไม่
...
<head>
  ...
  <style>
    /* กำหนดฟอนต์หลักทั่วโลก */
    body {
      font-family: 'Prompt', 'Poppins', sans-serif;
      margin: 0;
      padding: 0;
      background: #f0f4f8;
      /* สีพื้นหลังเบาๆ สำหรับหน้าที่มี Sidebar */
      color: #333;
    }
    ...
    /* ===== TOPBAR (HEADER) - ดีไซน์พาสเทล Glassmorphism ===== */
    #topbar {
      position: fixed;
      top: 0;
      left: 0;
    ...
    /* ===== CONTENT WRAPPER - ทั่วไป (มี Topbar) ===== */
    #content-wrapper {
    ...
    /* ===== ปรับให้หน้า login/register อยู่กึ่งกลาง (เมื่อมี Topbar) ===== */
    <?php if ($isPublicPage): ?>
      body {
        /* กำหนดให้ body เป็น flex เพื่อให้ content-wrapper (ซึ่งมี min-height: 100vh) อยู่ตรงกลาง */
        display: flex;
        flex-direction: column;
        min-height: 100vh;
        /* *** Background Animation ควรอยู่ในไฟล์ login.php/register.php *** */
      }
      ...
      #content-wrapper {
        ...
        display: flex;
        /* เพื่อจัด content ภายในให้กึ่งกลาง */
        justify-content: center;
        align-items: center;
        min-height: calc(100vh - var(--topbar-h)) !important;
        /* เต็มความสูงหน้าจอที่เหลือ */
      }
...
<body class="<?php echo $bodyClass; ?>">
  ...
  <?php if ($isAdmin):
    include __DIR__ . '/admin_sidebar.php'; endif; ?>
    ...
    <?php if (is_logged_in()):
      ...
      <div style="flex:1; display:flex; gap:15px; align-items:center;">
        ...
        <nav style="margin-left: auto; display:flex; gap:10px;">
          <?php if ($u['role'] === 'treasurer'): ?> <a href="treasurer_dashboard.php">
          Treasurer Dashboard</a><?php endif; ?>
          <a href="member_dashboard.php">Member Dashboard</a>
          </nav>
        <a href="logout.php" style="background-color: var(--secondary-pastel); font-weight: 600;">
          Logout</a>
        </div>
    <?php else: ?>
      <div style="flex:1; display:flex; justify-content: flex-end; gap: 10px;">
        <a href="login.php">Login</a>
        <a href="register.php" style="background-color: var(--primary-pastel); font-weight: 600;">ลงทะเบียน</a>
        </div>
    <?php endif; ?>
